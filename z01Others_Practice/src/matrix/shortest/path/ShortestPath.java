...
public class ShortestPath {
	...
	public static void main(String[] args) {
		...
		ShortestPath sp = new ShortestPath(new char[][] { 
			{ '1', '1', '1', '1', '1' },
			{ 'S', '1', 'X', '1', '1' }, 
			{ '1', '1', '1', '1', '1' },
			{ 'X', '1', '1', 'E', '1' }, 
	...
	private void next(Position position) {
		...
		if (shortestPath == null || path.size() < shortestPath.length) {
			if (!endFound(position)) {
				...
				System.out.println("If shortestPath "+Arrays.toString(shortestPath));

			} else {
				shortestPath = path.toArray(new Position[0]);
				System.out.println("Else shortestPath "+Arrays.toString(shortestPath));
			}
		}
